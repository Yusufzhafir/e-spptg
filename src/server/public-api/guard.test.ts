...
describe('authenticateApiRequest', () => {
  ...
  it('accepts a correct client id and key', () => {
    const result = authenticateApiRequest(request(VALID));
    expect(result).toEqual({ ok: true, clientId: 'dashboard-eksekutif' });
  });

  it('fails closed with 503 when no clients are configured', () => {
    delete process.env.STATISTIK_API_CLIENTS;
    const result = authenticateApiRequest(request(VALID));
    expect(result).toMatchObject({ ok: false, status: 503, kode: 'API_BELUM_DIKONFIGURASI' });
  });

  it('rejects a missing, unknown or wrong credential with 401', () => {
    expect(authenticateApiRequest(request())).toMatchObject({ status: 401 });
    expect(
    ...
    expect(
      authenticateApiRequest(request({ ...VALID, [API_KEY_HEADER]: 'salah' }))
    ).toMatchObject({ status: 401 });
  });

  it('rejects an unlisted IP once the allowlist is set', () => {
    process.env.STATISTIK_API_ALLOWED_IPS = '103.10.20.30';

    expect(
      authenticateApiRequest(request({ ...VALID, 'x-forwarded-for': '8.8.8.8' }))
  ...
  it('checks the IP before the credentials, so it cannot be used to probe client ids', () => {
    process.env.STATISTIK_API_ALLOWED_IPS = '103.10.20.30';
  ...
  it('throttles a client that floods the endpoint', () => {
    for (let i = 0; i < 120; i += 1) {
      expect(authenticateApiRequest(request(VALID)).ok).toBe(true);
    }

    const throttled = authenticateApiRequest(request(VALID));
  ...
  it('throttles per client, so one consumer cannot lock out another', () => {
    process.env.STATISTIK_API_CLIENTS =
      'dashboard-eksekutif:rahasia-panjang,bappeda:rahasia-lain';

    for (let i = 0; i < 121; i += 1) authenticateApiRequest(request(VALID));
