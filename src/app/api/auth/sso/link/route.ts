...
export async function POST(request: NextRequest) {
  ...
  if (!pending) return done(request, '/sign-in?sso=kadaluarsa');

  const form = await request.formData();
  if (form.get('aksi') !== 'hubungkan') {
    return done(request, '/sign-in?sso=dibatalkan');
  }
  ...
  if (!user) return done(request, '/sign-in?sso=gagal');
  if (user.status !== 'Aktif') return done(request, '/sign-in?sso=nonaktif');
  ...
  if (user.ssoSub && user.ssoSub !== pending.sub) {
    return done(request, '/sign-in?sso=gagal');
  }
  ...
  return done(request, safeNextPath(pending.next), setCookie);
}

/** 303 so the browser turns the POST into a GET on the destination. */
function done(request: NextRequest, target: string, setCookie?: string) {
