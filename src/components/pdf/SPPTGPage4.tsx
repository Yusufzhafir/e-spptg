 ...
 *
 * This component renders the map attachment of the SPPTG document containing:
 * - Map attachment header
 * - Owner metadata
 * - Land location map image
 *
 * The coordinate lists live on their own sheets — see `SPPTGCoordinatePage`.
 */
  ...
  return (
    <Page size="A4" style={styles.page}>
      ...
      <View style={[styles.section, { marginBottom: 8 }]}>
        ...
        {/* Only stated when there is more than one — a single-bidang certificate
            reads exactly as it always did. */}
        {polygons.length > 1 && (
          <View style={styles.row}>
            <Text style={styles.label}>Jumlah bidang</Text>
            <Text style={styles.colon}>:</Text>
            <Text style={styles.value}>{polygons.length} bidang</Text>
          </View>
        )}
        <View style={styles.row}>
          <Text style={styles.label}>Jumlah titik polygon</Text>
          <Text style={styles.colon}>:</Text>
          <Text style={styles.value}>{totalPoints} titik</Text>
        </View>
      </View>
